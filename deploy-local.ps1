...
#
# Wann lokal? Wenn das GitHub-Secret FTP_PASSWORD (noch) nicht gesetzt ist oder es
# sofort live muss. Normalweg bleibt GitHub Actions (.github/workflows/deploy.yml) -
# der Linux-Runner (curl + OpenSSL) laedt auch grosse Dateien sauber hoch.
#
# ⛔ NIE mit hochgeladen (siehe feste Dateiliste $rels unten - nur diese gehen live):
#    - *.local.txt  (v. a. OPERATOR-KEY.local.txt) — Betreiber-Schluessel nur lokal.
#    - supabase/    (Edge Functions — separat via Supabase deployen).
#    - gen_icons.py, deploy-local.*, .gitignore, .github/, .claude/.
#
...
#  - Windows-curl (Schannel) brach frueher beim TLS-Abbau der Datenverbindung ab
#    (ProFTPD: 450 Link lost, Datei verworfen). GELOEST: mit --tlsv1.2 --tls-max 1.2
#    gingen alle betroffenen Dateien durch (der TLS-1.2-Shutdown-Pfad in Schannel ist
#    sauber). Deshalb ist TLS 1.2 der STANDARD fuer den Paket-Upload; scheitert eine
#    Datei trotzdem, greifen der Reihe nach Ausweich-Transportwege, der erste
#    funktionierende wird beibehalten:
...
#
# Aufruf:
#   powershell -ExecutionPolicy Bypass -File "C:\Users\karbi\Documents\Claude Code\anaesthesieprotokoll\deploy-local.ps1"
...
$FTPDIR  = 'anaesthesieprotokoll'
$SITE    = 'https://anaesthesieprotokoll.vaydena.de'
...
# Feste Dateiliste = exakt das, was live gehoert (identisch zu deploy.yml).
$rels = @(
  'index.html',
  'anmelden.html',
  'app.html',
...
$errf = Join-Path $env:TEMP 'apt-curl-stderr.txt'
...
# --- Dateiliste einsammeln (feste Liste; fehlende Dateien werden gemeldet) ---
$files = @()
foreach ($r in $rels) {
  $p = Join-Path $root ($r -replace '/', '\')
  if (Test-Path $p) { $files += Get-Item $p }
  else { Log "  ⚠ fehlt lokal (uebersprungen): $r" }
}
$total = $files.Count
if ($total -eq 0) { Log "Keine Dateien gefunden - Abbruch."; $pw = $null; exit 1 }
...
# --- 2) Pakete von 6 Dateien; Ergebnis je Datei ueber -w %{exitcode} ---
$batchSize = 6
...
# --- 3) Ausweich-Transportwege fuer fehlgeschlagene Dateien ---
$strategies = @(
...
# --- 5) HTTPS-Verify (Textdateien: Groesse vergleichen; Medien: nur HTTP-Code) ---
